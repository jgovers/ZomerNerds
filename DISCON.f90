...
SUBROUTINE ColmanTransform(MOop1, MOop2, MOop3, aziAngle, axisDirect, axisQuadr)
!The Colman or d-q axis transformation transforms the out of plane moment of each turbine blade
!to a direct axis and a quadrature axis

	IMPLICIT NONE

	REAL(4), INTENT(IN)		:: MOop1, MOop2, MOop3				!out of plane moment of each blade
	...
	axisDirect	= 2/3 * cos(aziAngle)*MOop1 + cos(aziAngle+phi2)*MOop2 + cos(aziAngle+phi3)*MOop3
	axisQuadr	= 2/3 * cos(aziAngle)*MOop1 + cos(aziAngle+phi2)*MOop2 + cos(aziAngle+phi3)*MOop3

END SUBROUTINE ColmanTransform

SUBROUTINE ColmanTransformInverse(axisDirect, axisQuadr, aziAngle, delta1pAngle, MOop1, MOop2, MOop3)
!The inverse Colman or d-q axis transformation transforms the direct axis and quadrature axis
!back to the out of plane moment of each turbine blade
	...
	REAL(4), INTENT(OUT)	:: MOop1, MOop2, MOop3				!out of plane moment of each blade

	REAL(4), PARAMETER		:: PI = 3.14159265359				!mathematical constant pi
	REAL(4), PARAMETER		:: phi2 = 2/3*PI					!phase difference to second blade
	REAL(4), PARAMETER		:: phi3 = 4/3*PI					!phase difference to third blade

	MOop1 = cos(aziAngle+delta1pAngle)*axisDirect + sin(aziAngle+delta1pAngle)*axisQuadr
	MOop2 = cos(aziAngle+delta1pAngle+phi2)*axisDirect + sin(aziAngle+delta1pAngle+phi2)*axisQuadr
	MOop3 = cos(aziAngle+delta1pAngle+phi3)*axisDirect + sin(aziAngle+delta1pAngle+phi3)*axisQuadr
